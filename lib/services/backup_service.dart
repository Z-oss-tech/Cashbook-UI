...
class BackupService {
  ...
  static Future<bool> exportBackup(RecordProvider recordProvider, {bool isPreMigration = false}) async {
    try {
      final data = {
        'version': 1,
        'timestamp': DateTime.now().toIso8601String(),
        'cashbooks': recordProvider.cashbooks.map((c) => {
          'id': c.id,
          'name': c.name,
      ...
      final File file = File('${directory.path}/SmartKhata_Backup_$timestamp.json');
      ...
      await Share.shareXFiles([XFile(file.path)], text: 'SmartKhata Backup File');
  ...
  static Future<bool> restoreBackup(Map<String, dynamic> data, RecordProvider recordProvider) async {
    try {
  ...
  static Future<void> _recordBackupHistory(String type, int recordsCount, String sizeStr) async {
  ...
  static Future<List<Map<String, dynamic>>> getBackupHistory() async {
    final prefs = await SharedPreferences.getInstance();
    List<String> history = prefs.getStringList(_backupHistoryKey) ?? [];
    return history.map((e) => Map<String, dynamic>.from(jsonDecode(e))).toList();
  }
